...
#include <src/Edifor/Edifor.h>
#include <src/Edifor/Language.h>
...
struct editorSyntax HLDB[] = {
  ...
  {
    "FlaScript",
    FlaScript_HL_extensions,
    FlaScript_HL_keywords,
    "//", "/>", "</",
    HL_HIGHLIGHT_NUMBERS | HL_HIGHLIGHT_STRINGS
  },
  {
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t')
      rx += (EDIFOR_TAB_STOP - 1) - (rx % EDIFOR_TAB_STOP);
    rx++;
  }
  return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
  ...
  for (cx = 0; cx < row->size; cx++) {
    if (row->chars[cx] == '\t')
      cur_rx += (EDIFOR_TAB_STOP - 1) - (cur_rx % EDIFOR_TAB_STOP);
    cur_rx++;
...
void editorUpdateRow(erow *row) {
  ...
  free(row->render);
  row->render = malloc(row->size + tabs*(EDIFOR_TAB_STOP - 1) + 1);

  int idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % EDIFOR_TAB_STOP != 0) row->render[idx++] = ' ';
    } else {
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
          "Fegeya Edifor - %s", EDIFOR_VERSION);
